...
type DataProducer = rtrb::fixed_chunks::ChunkProducer<f32>;
type DataConsumer = rtrb::fixed_chunks::ChunkConsumer<f32>;
...
impl FileStreamer {
    pub fn new(
        ...
        let (seek_producer, mut seek_consumer) =
            rtrb::RingBuffer::<(u64, DataConsumer)>::new(1);
        let (mut data_producer, data_consumer): (DataProducer, _) =
            rtrb::RingBuffer::with_chunks(buffer_blocks as usize).of_size(chunksize as usize);
